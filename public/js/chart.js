...
async function drawChart(){
    ...
    for (var i=0 ; i<budegts.length;i++){
        ...
        const startDateParts = document.getElementById(`start-${budgetID}`).textContent.replace('Starting Date:', '').trim().split('/');
        const startDate = new Date(`${startDateParts[2]}-${startDateParts[0]}-${startDateParts[1]}`);
        const endDateParts = document.getElementById(`end-${budgetID}`).textContent.replace('Ending Date:', '').trim().split('/');
        ...
        const amount = document.getElementById(`amount-${budgetID}`).textContent.replace('Budget Amount: $', '').trim();
